...
namespace CraftSynth.BuildingBlocks.WindowsNT
{
	public class WindowsMessage
	{
		...
		public static int Send(int hWnd, int wParam, string msg)
		{
			int result = 0;
			...
			cds.lpData = msg;
			cds.cbData = len + 1;
			result = WindowsNTMethods.SendMessage(hWnd, WM_COPYDATA, wParam, ref cds);

			return result;
		}

		public static int Send(int hWnd, int Msg, int wParam, int lParam)
		{
			int result = WindowsNTMethods.SendMessage(hWnd, Msg, wParam, lParam);
			return result;
		}

		public static int Send(string targetWindowCaption, int Msg, int wParam, int lParam)
		{
			IntPtr targetWindow = Misc.GetWindowByCaption(targetWindowCaption);
			int result = Send((int)targetWindow, Msg, wParam, lParam);
			return result;
		}

		public static int SendCustomMessage(int targetWindow, int Msg, int wParam, int lParam)
		{
			int WM_CUSTOM_MESSAGE = WM_USER + Msg;
			int result = Send(targetWindow, WM_CUSTOM_MESSAGE, wParam, lParam);
			return result;
		}

		public static int SendCustomMessage(string targetWindowCaption, int Msg, int wParam, int lParam)
		{
			IntPtr targetWindow = Misc.GetWindowByCaption(targetWindowCaption);
			int WM_CUSTOM_MESSAGE = WM_USER + Msg;
			int result = Send((int)targetWindow, WM_CUSTOM_MESSAGE, wParam, lParam);
			return result;
		}

		//cant find window
		public static int SendCustomMessage(string targetWindowClassName, string targetWindowCaption, int Msg, int wParam, int lParam)
		{
			IntPtr targetWindow = Misc.GetWindowByClassNameAndCaption(targetWindowClassName, targetWindowCaption);
			int WM_CUSTOM_MESSAGE = WM_USER + Msg;
			int result = Send((int)targetWindow, WM_CUSTOM_MESSAGE, wParam, lParam);
			return result;
		}

		public static int GetCustomMessage(int Msg)
		{
			return Msg - WM_USER;
		}
	}
